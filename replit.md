...
- July 13, 2025. IMPLEMENTED: Server-side HEIC conversion system - Replaced client-side libraries with robust backend solution using heic-convert library and multer for file handling. Created /api/convert-heic endpoint that processes HEIC files server-side and returns converted JPEG data URLs. iPhone users can now upload HEIC files seamlessly with reliable conversion that works across all browsers and devices.
