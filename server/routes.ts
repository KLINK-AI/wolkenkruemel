      ...
      <body>
          <div class="container">
              ...
              <div class="test-section">
                  ...
                  <button onclick="testActivitiesDirectAPI()">Test Activities Direct API</button>
          ...
          <script>
              ...
              async function testActivitiesDirectAPI() {
                  const results = document.getElementById('api-results');
                  results.innerHTML = '<p>Teste Activities Direct API...</p>';
                  
                  try {
                      log('Teste Activities Direct API: ' + API_BASE + '/api/activities-direct');
                      const response = await fetch(\`\${API_BASE}/api/activities-direct\`);
                      ...
                      if (response.ok) {
                          ...
                          if (data.success) {
                              results.innerHTML = \`<p class="success">✅ Activities Direct API erfolgreich: \${data.count} Aktivitäten gefunden</p>\`;
                              log(\`Activities Direct API erfolgreich: \${data.count} Aktivitäten\`);
                              
                              if (data.activities && data.activities.length > 0) {
                                  const preview = data.activities.slice(0, 3).map(a => \`- \${a.title} (ID: \${a.id})\`).join('\\n');
                                  results.innerHTML += \`<p class="info">Beispiel-Aktivitäten:\\n\${preview}</p>\`;
                                  log('Beispiel-Aktivitäten: ' + preview);
                              }
                          } else {
                              results.innerHTML = \`<p class="error">❌ Activities Direct API fehlgeschlagen: \${data.error}</p>\`;
                              log(\`Activities Direct API fehlgeschlagen: \${data.error}\`, 'error');
                          }
                      } else {
                          const errorText = await response.text();
                          results.innerHTML = \`<p class="error">❌ Activities Direct API fehlgeschlagen: \${response.status}</p>\`;
                          
                          // Try to parse JSON error
                          try {
                              const errorJson = JSON.parse(errorText);
                              results.innerHTML += \`<p class="error">Message: \${errorJson.error || 'Unknown error'}</p>\`;
                              if (errorJson.stack) {
                                  results.innerHTML += \`<p class="error">Stack: \${errorJson.stack.substring(0, 200)}...</p>\`;
                              }
                          } catch {
                          ...
                          log(\`Activities Direct API fehlgeschlagen: \${response.status} - \${errorText}\`, 'error');
                      }
                  } catch (error) {
                      results.innerHTML = \`<p class="error">❌ Activities Direct API Fehler: \${error.message}</p>\`;
                      log(\`Activities Direct API Fehler: \${error.message}\`, 'error');
                  }
              }
